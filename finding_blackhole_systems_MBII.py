...
run='L205n625TNG'
save_output_path='/ufrc/lblecha/aklantbhowmick/Illustris_TNG_BHs/'
...
#--------------------------------------------------------------------------------------------
    ...
    # Simulation Box size: Default value set for Massive Black II. Note:- Periodic boundary conditions apply 
...
redshift_space=[0.0,0.6,1.0,1.5,2.0,2.5,3,3.5,4.0,4.5]
#redshift_space=[3,3.5]
log_LINKING_LENGTH_space=numpy.linspace(-2,2,40)
log_luminosity_cuts_space=[43.0,43.5,44.0,44.5,45.0,45.5]
log_bhmass_cuts_space=[8.0,7.0,6.0]
#log_luminosity_cuts_space=[42.5,42.0]

output_path_for_FOF_tags='/ufrc/lblecha/aklantbhowmick/FOF_tag_outputs/'
#output_path_for_RICHNESS='/ufrc/lblecha/aklantbhowmick/richness_outputs/'
...
#-------------------------This whole section reads the properties of objects----------------------------------          
    g=h5py.File(save_output_path+run+'bh_lum_host_halo_with_id_and_mass_z%.2f_and_sub_halo_id_added_velocity_all_BHs'%(redshift))
#    g2=h5py.File('/nfs/nas-0-1/akbhowmi/quasar_properties/bh_lum_host_halo_with_id_and_mass_z%.2f_and_halo_id_added_velocity'%redshift)
    ...
    hosthalo_id=g.get('host_halo_id')[:]
    ...
    for log_bhmass_cut in log_bhmass_cuts_space:
        ...
        #for log_LINKING_LENGTH in log_LINKING_LENGTH_space:
            ...
            if (PROJECTION_MODE):
                ...
                numpy.save(output_path_for_FOF_tags+'%s_log_bhmass_cut_%.1f_redshift_%.2f_log_LINKING_LENGTH_%.2f.npy'%(run,log_bhmass_cut,redshift,log_LINKING_LENGTH),[final_FOF_tag,subsampled_quantities])                #print(final_FOF_tag)
                #print("------------")
                
#                final_FOF_tag,subsampled_quantities=numpy.load(output_path_for_FOF_tags+'%s_log_bhmass_cut_%.1f_redshift_%.2f_log_LINKING_LENGTH_%.2f.npy'%(run,log_bhmass_cut,redshift,log_LINKING_LENGTH),allow_pickle=True)
#                position_vector_cut,velocity_vector_gadget_units_cut,blackhole_id_cut,subhalo_id_cut,hosthalo_id_cut=subsampled_quantities
